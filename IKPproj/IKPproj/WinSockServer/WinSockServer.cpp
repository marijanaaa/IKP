...
struct AddClientsInput {
	ProducerConsumer<ClientInfo>* Lobby;
	ProducerConsumer<char*>* ClientsQueue;
...
struct GameHandlerInput {
	ProducerConsumer<char*>* ClientsQueue;
	HashMap<ClientInfo>* Clients;
	ProducerConsumer<char*>* InGameQueue;
...
int main()
{
	...
	HANDLE ShutDownController = NULL;
	DWORD ShutDownControllerID;

	ShutDown shutDown = ShutDown();
	shutDown.FinishSignal = &FinishSignal;
	shutDown.serverWorking = &serverWorking;
	ShutDownController = CreateThread(NULL, 0, &ProcessAdminInput, (LPVOID)&shutDown, 0, &ShutDownControllerID);
	...
	ProducerConsumer<char*> clientsQueue(&FinishSignal, 50);
...
DWORD WINAPI ProcessAdminInput(LPVOID param)
{
	...
	while (serverWorking)
	{
		if (_kbhit)
		{
			...
			{
				*serverWorking = 0;
				ReleaseSemaphore(*(data.FinishSignal), 9, NULL);
...
int ClearClientsMap(HashMap<ClientInfo>* clients) {
	int keysCount = 0;
	char** keys = clients->GetKeys(&keysCount);
...
DWORD WINAPI FeedClientsToProcess(LPVOID param) {
	...
	while (WaitForSingleObject(*input->FinishSignal, 0) != WAIT_OBJECT_0) {
		...
		for (int i = 0; i < count;i++) {
    		int result = input->ClientsQueue->Produce(copy[i]);
			if (result == 1) {
				LeaveCriticalSection(input->GameSetup);
				Sleep(100);
				EnterCriticalSection(input->GameSetup);
...
DWORD WINAPI GameHandler(LPVOID param) {
	...
	while (WaitForSingleObject(*input->FinishSignal, 0) != WAIT_OBJECT_0) {
		...
		ClientInfo clientInfo;
		bool isExist = input->Clients->Get(clientId, &clientInfo);
		LeaveCriticalSection(input->GameSetup);
		...
		if (data.Type == READY) {
			...
			EnterCriticalSection(input->GameSetup);
			input->Clients->Insert(clientId, clientInfo);
			LeaveCriticalSection(input->GameSetup);
